...
use rustler::{Atom, Binary, Encoder, Env, Resource, ResourceArc, Term};
...
fn decode_score_bound(term: Term) -> Result<Bound<Score>, rustler::Error> {
    ...
    let tuple = term.decode::<(Atom, f64)>()?;
...
fn decode_lex_bound(term: Term) -> Result<Bound<storage::Bytes>, rustler::Error> {
    ...
    let tuple = term.decode::<(Atom, Binary)>()?;
...
fn execute_single_write_command<'a>(
    ...
    if let Ok(terms) = terms {
        ...
        if let Ok(cmd_terms) = inner_tuple {
            ...
            let cmd_atom: Result<Atom, _> = cmd_terms[0].decode();
            ...
            if let Ok(cmd_atom) = cmd_atom {
                ...
                if cmd_atom == atoms::set() && arg_count == 2 {
                    ...
                    if let (Ok(key), Ok(value)) =
                        (args[0].decode::<Binary>(), args[1].decode::<Binary>())
                    {
                    ...
                    if let (Ok(key), Ok(target_db)) =
                        (args[0].decode::<Binary>(), args[1].decode::<u64>())
                    {
                    ...
                    if let (Ok(key), Ok(increment)) =
                        (args[0].decode::<Binary>(), args[1].decode::<i64>())
                    {
                    ...
                    if let (Ok(key), Ok(members)) =
                        (args[0].decode::<Binary>(), args[1].decode::<Vec<Binary>>())
                    {
                        let members_slices: Vec<&[u8]> =
                            members.iter().map(|b| b.as_slice()).collect();
                        ...
                        (args[0].decode::<Binary>(), args[1].decode::<Vec<Binary>>())
                    {
                        let keys_slices: Vec<&[u8]> =
                            source_keys.iter().map(|b| b.as_slice()).collect();
                    ...
                    if let (Ok(key), Ok(values)) =
                        (args[0].decode::<Binary>(), args[1].decode::<Vec<Binary>>())
                    {
                        let values_slices: Vec<&[u8]> =
                            values.iter().map(|b| b.as_slice()).collect();
                    ...
                    if let (Ok(source_key), Ok(dest_key), Ok(wherefrom), Ok(whereto)) =
                        (source_key, dest_key, wherefrom, whereto)
                    {
                        let from_left = wherefrom == atoms::left();
                        let to_left = whereto == atoms::left();
                        return match inner.lmove(
                            db,
                            source_key.as_slice(),
                            dest_key.as_slice(),
                            from_left,
                    ...
                    if let (Ok(key), Ok(direction), Ok(pivot), Ok(value)) = (
                        ...
                        let before = if direction == atoms::before() {
                            ...
                            return (atoms::error(), rustler::types::atom::error().encode(env))
                                .encode(env);
                        };
                        return match inner.linsert(
                            db,
                            key.as_slice(),
                            before,
                            pivot.as_slice(),
                    ...
                    if let (Ok(key), Ok(fields)) =
                        (args[0].decode::<Binary>(), args[1].decode::<Vec<Binary>>())
                    {
                        let fields_slices: Vec<&[u8]> =
                            fields.iter().map(|f| f.as_slice()).collect();
                    ...
                    if let Ok(key) = args[0].decode::<Binary>() {
                        ...
                        let members_result: Result<Vec<(f64, Binary)>, rustler::Error> =
                            args[1].decode::<Vec<Term>>().and_then(|terms| {
                                terms
                                    .iter()
                                    .map(|term| {
                    ...
                    if let (Ok(key), Ok(count)) =
                        (args[0].decode::<Binary>(), args[1].decode::<usize>())
                    {
                    ...
                    if let Ok(key) = args[0].decode::<Binary>() {
                        if let (Ok(min_bound), Ok(max_bound)) =
                            (decode_score_bound(args[1]), decode_score_bound(args[2]))
                        {
                            return match inner.zremrangebyscore(
                                db,
                                key.as_slice(),
                                min_bound,
                                max_bound,
                    ...
                    if let Ok(key) = args[0].decode::<Binary>() {
                        if let (Ok(min_bound), Ok(max_bound)) =
                            (decode_lex_bound(args[1]), decode_lex_bound(args[2]))
                        {
                            return match inner.zremrangebylex(
                                db,
                                key.as_slice(),
                                min_bound,
                                max_bound,
                    ...
                    if let (Ok(dest_key), Ok(source_keys), Ok(weights), Ok(aggregate_atom)) = (
                        ...
                        let keys_slices: Vec<&[u8]> =
                            source_keys.iter().map(|k| k.as_slice()).collect();
                        return match inner.zunionstore(
                            db,
                            dest_key.as_slice(),
                            &keys_slices,
                            &weights,
                    ...
                    if let (Ok(dest_key), Ok(source_keys), Ok(weights), Ok(aggregate_atom)) = (
                        ...
                        let keys_slices: Vec<&[u8]> =
                            source_keys.iter().map(|k| k.as_slice()).collect();
                        return match inner.zinterstore(
                            db,
                            dest_key.as_slice(),
                            &keys_slices,
                            &weights,
                    ...
                    if let (Ok(db1), Ok(db2)) = (args[0].decode::<u64>(), args[1].decode::<u64>()) {
            ...
            if cmd_atom == atoms::get() && arg_count == 1 {
                ...
                if let Ok(key) = args[0].decode::<Binary>() {
                    return match inner.smembers(db, key.as_slice()) {
                        Ok(members) => {
                            let binaries: Vec<Binary> = members
                                .iter()
                                .map(|m| {
                                    let mut binary =
                ...
                if let (Ok(key), Ok(count)) = (args[0].decode::<Binary>(), args[1].decode::<i64>())
                {
                ...
                if let (Ok(key), Ok(start), Ok(stop)) = (
                    ...
                    return match inner.lrange(db, key.as_slice(), start, stop) {
                        Ok(elements) => {
                            let binaries: Vec<Binary> = elements
                                .iter()
                                .map(|e| {
                                    let mut binary =
                ...
                if let Ok(key) = args[0].decode::<Binary>() {
                    return match inner.hgetall(db, key.as_slice()) {
                        Ok(pairs) => {
                            let tuples: Vec<(Binary, Binary)> = pairs
                                .iter()
                                .map(|(f, v)| {
                                    let mut field_bin =
...
fn lua_load<'a>(env: Env<'a>, storage: ResourceArc<TStorage>, script: Binary) -> Term<'a> {
    // Lock the storage
    let inner = match storage.0.lock() {
        Ok(guard) => guard,
        Err(poisoned) => poisoned.into_inner(),
