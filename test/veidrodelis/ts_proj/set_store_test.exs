defmodule Vdr.TSProj.SetStoreTest do
  ...
  describe "sadd/4" do
    test "adds members to a set", %{storage: storage} do
      assert :ok = Vdr.TS.sadd(storage, 0, "myset", ["a", "b", "c"])
    ...
    test "adding duplicate members is idempotent", %{storage: storage} do
      assert :ok = Vdr.TS.sadd(storage, 0, "myset", ["a", "b"])
      assert :ok = Vdr.TS.sadd(storage, 0, "myset", ["b", "c"])
    ...
    test "supports multiple databases", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "myset", ["a", "b"])
      Vdr.TS.sadd(storage, 1, "myset", ["x", "y"])
    ...
    test "adds empty list of members", %{storage: storage} do
      assert :ok = Vdr.TS.sadd(storage, 0, "myset", [])
    ...
    test "returns error on type mismatch", %{storage: storage} do
      ...
      assert {:error, :wrong_type} = Vdr.TS.sadd(storage, 0, "mystring", ["a"])
    end
  end

  describe "srem/4" do
    test "removes members from a set", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "myset", ["a", "b", "c", "d"])
      assert :ok = Vdr.TS.srem(storage, 0, "myset", ["b", "d"])
    ...
    test "removing non-existent members is safe", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "myset", ["a", "b"])
      assert :ok = Vdr.TS.srem(storage, 0, "myset", ["c", "d"])
    ...
    test "removes all members and deletes key", %{storage: storage} do
      ...
      assert :ok = Vdr.TS.srem(storage, 0, "myset", ["a", "b", "c"])
    ...
    test "returns error on type mismatch", %{storage: storage} do
      Vdr.TS.set(storage, 0, "mystring", "value")
      assert {:error, :wrong_type} = Vdr.TS.srem(storage, 0, "mystring", ["a"])
    end
  end
  ...
  describe "smove/5" do
    test "moves member from source to destination", %{storage: storage} do
      ...
      assert :ok = Vdr.TS.smove(storage, 0, "set1", "set2", "b")
    ...
    test "moves member and deletes source if it becomes empty", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "set1", ["a"])
      Vdr.TS.sadd(storage, 0, "set2", ["x"])

      assert :ok = Vdr.TS.smove(storage, 0, "set1", "set2", "a")
    ...
    test "succeeds even if member doesn't exist", %{storage: storage} do
      ...
      assert :ok = Vdr.TS.smove(storage, 0, "set1", "set2", "z")
    ...
    test "returns error on type mismatch", %{storage: storage} do
      Vdr.TS.set(storage, 0, "mystring", "value")
      Vdr.TS.sadd(storage, 0, "myset", ["a"])

      assert {:error, :wrong_type} = Vdr.TS.smove(storage, 0, "mystring", "myset", "a")
      assert {:error, :wrong_type} = Vdr.TS.smove(storage, 0, "myset", "mystring", "a")
    end
  end
  ...
  describe "sunionstore/4" do
    test "stores union of multiple sets", %{storage: storage} do
      ...
      Vdr.TS.sadd(storage, 0, "set3", ["c", "d"])

      assert :ok = Vdr.TS.sunionstore(storage, 0, "result", ["set1", "set2", "set3"])
    ...
    test "handles non-existent source sets", %{storage: storage} do
      ...
      assert :ok = Vdr.TS.sunionstore(storage, 0, "result", ["set1", "nonexistent"])
    ...
    test "returns error on type mismatch", %{storage: storage} do
      ...
      assert {:error, :wrong_type} = Vdr.TS.sunionstore(storage, 0, "result", ["myset", "mystring"])
    end
  end
  ...
  describe "sinterstore/4" do
    test "stores intersection of multiple sets", %{storage: storage} do
      ...
      Vdr.TS.sadd(storage, 0, "set2", ["b", "c", "e", "f"])
      Vdr.TS.sadd(storage, 0, "set3", ["c", "d", "g"])

      assert :ok = Vdr.TS.sinterstore(storage, 0, "result", ["set1", "set2", "set3"])
    ...
    test "handles empty intersection", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "set1", ["a", "b"])
      Vdr.TS.sadd(storage, 0, "set2", ["c", "d"])

      assert :ok = Vdr.TS.sinterstore(storage, 0, "result", ["set1", "set2"])
    ...
    test "returns error on type mismatch", %{storage: storage} do
      ...
      assert {:error, :wrong_type} = Vdr.TS.sinterstore(storage, 0, "result", ["myset", "mystring"])
    end
  end

  describe "sdiffstore/4" do
    test "stores difference of sets", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "set1", ["a", "b", "c", "d", "e"])
      Vdr.TS.sadd(storage, 0, "set2", ["b", "d"])
      Vdr.TS.sadd(storage, 0, "set3", ["c"])

      assert :ok = Vdr.TS.sdiffstore(storage, 0, "result", ["set1", "set2", "set3"])
    ...
    test "handles non-existent subtract sets", %{storage: storage} do
      ...
      assert :ok = Vdr.TS.sdiffstore(storage, 0, "result", ["set1", "nonexistent"])
    ...
    test "returns error on type mismatch", %{storage: storage} do
      ...
      assert {:error, :wrong_type} = Vdr.TS.sdiffstore(storage, 0, "result", ["myset", "mystring"])
    end
  end
  ...
  describe "scard/3" do
    ...
    test "returns 0 after removing all members", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "myset", ["a", "b"])
      Vdr.TS.srem(storage, 0, "myset", ["a", "b"])
  ...
  describe "binary safety" do
    test "handles binary members with null bytes", %{storage: storage} do
      ...
      Vdr.TS.sadd(storage, 0, "binset", [binary_member, "normal"])
    ...
    test "handles UTF-8 members", %{storage: storage} do
      Vdr.TS.sadd(storage, 0, "utf8set", ["hello", "世界", "🚀"])
